  ...
  Future<List<Task>> getTasks(int personId) async {
    if (filter.value.isEmpty) {
      if (showCompleted.value) {
        tasks = await Task().select().personId.equals(personId).orderBy([
          'isCompleted',
          'dueDate',
        ]).toList();
      } else {
      ...
      tasks = await Task()
          .select()
          .where(
            "personId = ? ${!showCompleted.value ? 'AND isCompleted = false' : ''}  AND (title LIKE ? OR description LIKE ?)",
            parameterValue: [
              personId,
              '%${filter.value}%',
              '%${filter.value}%',
